...
function createPendahuluan() {
  return [
    ...
    // ── PARAGRAF 1: Definisi masalah → Potensi/peluang → Data kuantitatif → Kesimpulan ──
    para([
      normalText('Perjudian online ('),
      normalText('online gambling', { italic: true }),
      normalText(') telah menjadi salah satu tantangan paling serius terhadap stabilitas dan integritas sistem keuangan nasional Indonesia. Perjudian online dapat dikategorikan sebagai sumber kejahatan keuangan sistemik yang menjadi faktor risiko utama terhadap pencucian uang ('),
      normalText('money laundering', { italic: true }),
      normalText('), pendanaan terorisme, dan penyalahgunaan infrastruktur perbankan. Aktivitas ini sering kali berhubungan dengan jaringan rekening '),
      normalText('mule', { italic: true }),
      normalText(' dan praktik '),
      normalText('layering', { italic: true }),
      normalText(' yang dapat memperburuk kondisi keamanan finansial secara keseluruhan '),
      cite('Suresh & Kumar', '2023'),
      normalText('. Menurut data Pusat Pelaporan dan Analisis Transaksi Keuangan (PPATK) tahun 2024, total perputaran dana perjudian online yang teridentifikasi melalui sistem perbankan Indonesia mencapai Rp 100 triliun, di mana lebih dari 5.000 rekening terindikasi sebagai saluran dana ilegal dengan peningkatan sebesar 215% dibandingkan periode sebelumnya '),
      cite('PPATK', '2024'),
      normalText('. Hingga saat ini, belum ditemukan platform terintegrasi yang mampu melakukan deteksi dan penegakan secara otomatis dan '),
      normalText('real-time', { italic: true }),
      normalText(' terhadap transaksi perjudian online dalam skala nasional. Namun, kemajuan dalam bidang kecerdasan buatan, khususnya '),
      normalText('Graph Neural Network', { italic: true }),
      normalText(' (GNN), memberikan peluang signifikan untuk menganalisis pola jaringan transaksi yang kompleks dan mengidentifikasi anomali secara lebih akurat dibandingkan metode konvensional '),
      cite('Kipf & Welling', '2017'),
      normalText('.'),
    ]),

    // ── PARAGRAF 2: Kondisi saat ini → Faktor penyebab → Dampak ──
    para([
      normalText('Kesadaran dan kapasitas pengawasan terhadap transaksi perjudian online di Indonesia yang masih terbatas menjadi salah satu faktor utama meningkatnya prevalensi kejahatan keuangan ini. Rendahnya efektivitas pengawasan berhubungan signifikan dengan ketergantungan pada sistem deteksi berbasis aturan ('),
      normalText('rule-based', { italic: true }),
      normalText(') yang hanya mampu mengenali pola yang telah didefinisikan sebelumnya, kurangnya integrasi data lintas lembaga, dan proses penindakan manual yang memakan waktu berhari-hari '),
      cite('Weber et al.', '2023'),
      normalText('. Selain itu, modus operandi pelaku yang semakin canggih — menggunakan teknik '),
      normalText('layering', { italic: true }),
      ...
      normalText('structuring', { italic: true }),
      normalText(' (pengaturan nominal di bawah ambang batas pelaporan) — berkontribusi besar terhadap kompleksitas deteksi yang melampaui kemampuan sistem konvensional '),
      cite('Suresh & Kumar', '2023'),
      normalText('. Dampak dari rendahnya kapasitas pengawasan dan evolusi modus operandi ini sangat signifikan, karena pelaku yang tidak terdeteksi secara dini cenderung memperluas jaringan rekening '),
      normalText('mule', { italic: true }),
      normalText(' hingga melibatkan ratusan node yang saling terhubung, yang dapat mengakibatkan kerugian finansial masif, pencucian uang lintas yurisdiksi, kerusakan reputasi lembaga keuangan, dan terganggunya stabilitas sektor jasa keuangan secara keseluruhan '),
      cite('Chen et al.', '2024'),
      normalText('.'),
    ]),

    // ── PARAGRAF 3: Upaya Pemerintah → Kekurangan dari upaya pemerintah ──
    para([
      normalText('Beberapa upaya telah dilakukan oleh pemerintah Indonesia untuk menangani masalah perjudian online dan dampaknya terhadap sistem keuangan. Otoritas Jasa Keuangan (OJK) telah menerbitkan Peraturan OJK (POJK) tentang Penerapan Program Anti Pencucian Uang dan Pencegahan Pendanaan Terorisme (APU-PPT) yang mewajibkan lembaga jasa keuangan melakukan pemantauan transaksi mencurigakan dan pemblokiran rekening terindikasi '),
      cite('OJK', '2023'),
      normalText('. Kementerian Komunikasi dan Digital (Kemenkomdigi) juga aktif melakukan pemblokiran terhadap lebih dari 3.700 situs dan aplikasi perjudian online sepanjang tahun 2024, sementara PPATK menjalankan fungsi analisis transaksi keuangan mencurigakan untuk mendukung penegakan hukum '),
      cite('Kemenkomdigi', '2024'),
      normalText('. Meski demikian, program-program ini masih menghadapi kendala operasional yang signifikan: proses analisis transaksi yang memerlukan waktu berhari-hari, koordinasi lintas lembaga yang kompleks antara OJK, bank, Kemenkomdigi, dan aparat penegak hukum, keterbatasan sumber daya manusia untuk menganalisis volume transaksi yang masif, serta tidak adanya sistem terintegrasi yang menghubungkan deteksi anomali dengan eksekusi pembekuan secara '),
      normalText('end-to-end', { italic: true }),
      normalText(' '),
      cite('Hidayat & Nugroho', '2024'),
      normalText('. Kekurangan ini mendorong perlunya pendekatan yang lebih inovatif, otomatis, dan terintegrasi berbasis teknologi kecerdasan buatan.'),
    ]),

    // ── PARAGRAF 4: Solusi alternatif (judul karya) → 5W+1H secara singkat ──
    para([
      normalText('Solusi alternatif dari seluruh permasalahan yang telah dibahas adalah '),
      normalText('NEXA AI: Inovasi Platform Deteksi dan Penegakan Perjudian Online Berbasis Graph Neural Network untuk Penguatan Pengawasan Sistem Keuangan di Otoritas Jasa Keuangan', { bold: true }),
      normalText('. Inovasi NEXA AI berfokus pada pengembangan platform terintegrasi ('),
      normalText('what', { italic: true }),
      normalText(') yang dirancang untuk Otoritas Jasa Keuangan sebagai regulator sektor keuangan nasional ('),
      normalText('who', { italic: true }),
      normalText(') dalam rangka mendeteksi dan menindak transaksi perjudian online secara '),
      normalText('real-time', { italic: true }),
      normalText(' dan otomatis ('),
      normalText('why', { italic: true }),
      normalText('). Platform ini beroperasi pada jaringan perbankan nasional yang diawasi oleh OJK ('),
      normalText('where', { italic: true }),
      normalText(') dan diimplementasikan sebagai solusi jangka panjang yang dapat diadopsi secara bertahap mulai dari fase simulasi hingga produksi penuh ('),
      normalText('when', { italic: true }),
      normalText('). NEXA AI bekerja dengan memanfaatkan teknologi '),
      normalText('Graph Neural Network', { italic: true }),
      normalText(' (GNN) yang mampu memodelkan hubungan antar-entitas dalam bentuk graf untuk mendeteksi pola transaksi anomali, dilengkapi dengan protokol '),
      normalText('Mutual TLS', { italic: true }),
      normalText(' (mTLS) yang memungkinkan integrasi langsung dan aman ke '),
      normalText('core-banking', { italic: true }),
      normalText(' API perbankan untuk eksekusi pembekuan rekening '),
      normalText('mule', { italic: true }),
      normalText(' secara simultan, serta arsitektur '),
      normalText('microservices', { italic: true }),
      normalText(' yang menjamin skalabilitas dan keandalan sistem ('),
      normalText('how', { italic: true }),
      normalText(') '),
      cite('Kipf & Welling', '2017'),
      normalText('; '),
      cite('Rescorla', '2018'),
      normalText('. Dengan demikian, NEXA AI tidak hanya membantu dalam deteksi dan penegakan perjudian online, tetapi juga berkontribusi pada penguatan kapasitas pengawasan OJK dan perlindungan integritas sistem keuangan nasional.'),
    ]),
